...
# fix php configs
sed -i -e 's/;cgi\.fix_pathinfo=1/cgi.fix_pathinfo=0/' /etc/opt/remi/php73/php.ini
sed -i -e 's|listen = 127.0.0.1:9000|listen = /run/php-fpm/www.sock|' /etc/opt/remi/php73/php-fpm.d/www.conf
sed -i -e 's/;listen.acl_users = apache/listen.acl_users = nginx/' /etc/opt/remi/php73/php-fpm.d/www.conf
sed -i -e 's/user = apache/user = nginx/' /etc/opt/remi/php73/php-fpm.d/www.conf
sed -i -e 's/group = apache/group = nginx/' /etc/opt/remi/php73/php-fpm.d/www.conf

# copy nginx config, systemd units, and app
/usr/bin/cp -u /home/git/vitae-net/deploy/nginx.conf /etc/nginx/nginx.conf
/usr/bin/cp -u /home/git/vitae-net/deploy/laravel-echo-server.service /usr/lib/systemd/system
/usr/bin/cp -u /home/git/vitae-net/deploy/vitae-net-queue.service /usr/lib/systemd/system
...
mkdir -p /var/www
/usr/bin/cp -ruf /home/git/vitae-net/. /var/www/vitae-net
